...
> 來源：EXP-000 Harness Replay Test。這些規則已在單一 DON ACC. Replay 中展現價值，但尚未經第二個不同專案驗證，因此目前是 candidate rules，不進 `donix/adopted-methods.md`。
...
- DON ACC. Replay：已驗證一次
- 第二個不同類型專案：未驗證
- 正式 donix methodology：未採用

至少需在 EXP-001 或另一個不同類型的 donix 專案再次驗證，才能考慮升級。