...
> donix 最有價值的品牌、商業、UX 判斷，能否以低維護成本被結構化、重用，並提升下一個專案的 AI 執行品質？
...
DON ACC. 官網 AI-native 重構。

使用已存在的品牌／商業最終判斷建立 Human-only Frozen Ground Truth；Replay Agent 只能看到 Minimum Context Pack，不得讀 Ground Truth 或舊版最終 Vibe Coding Prompt。
...
- Frozen Ground Truth：`EXP-000-ground-truth-donacc-v1.md`
- Codex execution prompt：`EXP-000-codex-execution-prompt.md`
- Round A Human Gate：`EXP-000-round-a-human-gate.md`
- Round B Human Gate：`EXP-000-round-b-human-gate.md`
- Final score：`EXP-000-scorecard.md`
- Candidate rules：`EXP-000-candidate-rules.md`
- Final result：`EXP-000-result.md`
...
4. Project knowledge 應該自足，不依賴 ChatGPT memory、個人化指令或「之前聊過」。
...
**允許進入 EXP-001｜donix Product Delivery Harness。**

但此結果只驗證到 Spec 層，不得直接宣稱完整 AI-native delivery harness 已成熟。EXP-001 必須另行驗證 `Approved Spec → Coding → QA → Human Gate → Harness Review`。
